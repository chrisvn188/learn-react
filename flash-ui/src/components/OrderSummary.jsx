...
export default function OrderSummary() {
  const [currentCartProducts, setCurrentCartProducts] = useState(cartProducts)
  ...
  const subTotal =
    currentCartProducts
      .map((product) => Number(product.price.slice(1)))
      .reduce(sum, 0) || 0

  const total = subTotal + shippingCost || 0

  const handleDeleteProduct = (id) => {
    setCurrentCartProducts(
      currentCartProducts.filter((product) => product.id !== id)
    )
  }
